...
class Discriminator_PatchGAN(nn.Module):
    def __init__(self, input_nc=3, ndf=64, n_layers=3, norm_type='spectral'):
        ...
        for n in range(len(sequence)):
            self.model.add_module(f'child{str(n)}', nn.Sequential(*sequence[n]))

        self.model.apply(self.weights_init)

    def use_spectral_norm(self, module, norm_type='spectral'):
        return spectral_norm(module) if 'spectral' in norm_type else module
    ...
    def get_norm_layer(self, norm_type='instance'):
        if 'batch' in norm_type:
            ...
            return functools.partial(nn.Identity)
...
class Discriminator_VGG_96(nn.Module):
    def __init__(self, in_nc=3, base_nc=64, ac_type='BL'):
        ...
        conv1 = B.conv(base_nc, base_nc, kernel_size=4, stride=2, mode=f'C{ac_type}')
        # 48, 64
        conv2 = B.conv(base_nc, base_nc*2, kernel_size=3, stride=1, mode=f'C{ac_type}')
        conv3 = B.conv(
            base_nc * 2, base_nc * 2, kernel_size=4, stride=2, mode=f'C{ac_type}'
        )

        # 24, 128
        conv4 = B.conv(
            base_nc * 2, base_nc * 4, kernel_size=3, stride=1, mode=f'C{ac_type}'
        )

        conv5 = B.conv(
            base_nc * 4, base_nc * 4, kernel_size=4, stride=2, mode=f'C{ac_type}'
        )

        # 12, 256
        conv6 = B.conv(
            base_nc * 4, base_nc * 8, kernel_size=3, stride=1, mode=f'C{ac_type}'
        )

        conv7 = B.conv(
            base_nc * 8, base_nc * 8, kernel_size=4, stride=2, mode=f'C{ac_type}'
        )

        # 6, 512
        conv8 = B.conv(
            base_nc * 8, base_nc * 8, kernel_size=3, stride=1, mode=f'C{ac_type}'
        )
